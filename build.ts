...
const TSC_ARGS = (config = 'build') => [`-p`, `${process.cwd()}/src/tsconfig-${config}.json`];
...
function createUmd(globals) {
  const name = 'angular-bootstrap-md';
  const entry = `${process.cwd()}/dist/es5/index.js`;
  return generateBundle(
    entry,
    `${process.cwd()}/dist/packages-dist/${name}.umd.js`,
    globals,
    name,
    'umd',
  );
...
function createEs(globals, target) {
  ...
  return generateBundle(
    entry,
    `${process.cwd()}/dist/packages-dist/${name}.${target}.js`,
    globals,
    name,
    'es',
  );
